...
export class WorkerManager {
    ...
    private setup(): void {
        this._io.on('connection', (socket: Socket) => {
            ...
            socket.on('register', (data: { id: string, address: string, job_id: string; status: string }) => {
                this.registerWorker(socket, data);
            });

            socket.on('heartbeat', (data: { id: string, address: string, job_id: string; status: string }) => {
    ...
    private registerWorker(socket: Socket, data: { id: string, address: string, job_id: string; status: string }): void {
    ...
    private updateHeartbeat(socket: Socket, data: { id: string, address: string, job_id: string; status: string }): void {
